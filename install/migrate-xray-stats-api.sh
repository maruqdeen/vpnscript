...
# "Check Active User" (vmess/vless/trojan) to query live per-user traffic
# counters. NON-DESTRUCTIVE: existing inbounds/clients are untouched.
# Backs up the live config first and automatically rolls back if Xray
# fails to restart with the new blocks, since this touches a server
