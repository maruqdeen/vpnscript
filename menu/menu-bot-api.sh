...
# Bot & Api Setup submenu. Menu shell only — functionality intentionally
# not implemented yet, per explicit instruction to hold off until the
# exact behavior is specified.
set -uo pipefail
...
while true; do
  ...
  case "$opt" in
    1) echo "Connect to TelegramBot — not built yet." ; pause ;;
